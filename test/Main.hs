...
wmTwoWindows = wmBlankState { wmWindows      = [win0, win1]
                            , wmFocusHistory = [wid0, wid1]
                            , wmDragResize   = Nothing
                            }

mapsWindow :: Test
mapsWindow = sequenceTests
  wmBlankState
  [ ( EvWantsMap win0
    , \wm cs ->
...
snap2Wins =
  ...
  in  "simple snapping cases"
        ~: [ "window snaps to single opposing edge" ~: dragMoveResizeTest
             wm0
             w1
             (30 , 110)
             (280, 250)
             (Bounds 277 297 240 260)
           , "window snaps to single same edge (left)" ~: dragMoveResizeTest
             wm0
             w1
             (30 , 110)
             (330, 250)
...
resizeAWindow =
  ...
  in
    "single window resize, no snap"
      ~: [ "drag right handle" ~: dragMoveResizeTest wm0
                                                     w1
                                                     (195, 350)
                                                     (215, 360)
                                                     (Bounds 100 220 300 400)
         , "drag left handle" ~: dragMoveResizeTest wm0
                                                    w1
                                                    (105, 350)
                                                    (95 , 360)
                                                    (Bounds 90 200 300 400)
         , "drag top handle" ~: dragMoveResizeTest wm0
                                                   w1
                                                   (150, 305)
                                                   (160, 295)
                                                   (Bounds 100 200 290 400)
         , "drag bottom handle" ~: dragMoveResizeTest wm0
                                                      w1
                                                      (150, 395)
                                                      (160, 405)
...
wm3Windows :: WmState
wm3Windows = wmBlankState { wmWindows      = [win0, win1, win2]
...
mruFocusSwitching =
  ...
  in
    "mru focus switching" ~: sequenceTests
      wm0
